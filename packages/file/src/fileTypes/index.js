import { getFileExtension, formatFileExtension } from '../utils'
...
function imageFormatHasExtension(imageFormat, extension) {
  ...
  return !!imageFormat.fileExtensions.find(imageFormatExtension => {
    return formatFileExtension(imageFormatExtension, true) === extension
  })
}
...
export function getBestImageFormatForFile(filePath, options) {
  const imageFormatsName = getBestImageFormatNameForFile(filePath, options)
  return getImageFormat(imageFormatsName)
}